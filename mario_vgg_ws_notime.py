...
def bot_play(mainDQN, env=env):
    ...
    while True:
        ...
        for n in range(len(action)):
            state, reward, done, info = env._step(action[n])
            if done == True:
                break
        reward_sum += reward
        if done:
 ...
 #               saver.restore(sess, checkpoint_dir + 'model_notime.ckpt')
            ...
            for episode in range(init_episode, max_episodes):
                ...
                while not done:
                    ...
                    if np.random.rand(1) < e or state is None or state.size == 1 or _count<=10:
                    ...
                    for n in range(len(action)):
                        next_state, reward, done, info = env._step(action[n])
                        if done == True:
                            print("%dth:", n)
                            break
                    print("dms~~",reward)
