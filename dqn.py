...
class DQN:
    ...
    def _build_network(self):
        ...
        with tf.variable_scope(self.net_name):
                ...
                self._P = tf.placeholder(tf.float32, [None,  6, 5], name="input_p")
                ...
                with tf.variable_scope("action_value"):
                    ...
                    out = tf.concat([out, JOYSTICK_Layer], axis=1)
                    out = layers.fully_connected(out, num_outputs=512,         activation_fn=tf.nn.relu)
                    self._Qpred = layers.fully_connected(out, num_outputs=self.output_size , activation_fn=None)
...
#                    self._Qpred = tf.layers.dense(VGG_Layer6_3, units=self.output_size, use_bias=False)
    ...
    def predict(self, state, action_seq):
        ...
        action_seq = np.reshape(action_seq, [1, 6, 5])
