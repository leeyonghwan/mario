...
#    input_shape = env.observation_space.shape[0]*env.observation_space.shape[1]*3
    ...
    t_start = time.time()
    max_distance = 0
    for t in range(5000):#itertools.count():
...
#        e = 1. / ((t / 20) + 1)
        ...
        # YOUR CODE HERE
            ...
            if not model_initialized or np.random.rand() <= exploration.value(t):
                action = replay_buffer.encode_action_rand()
            else:
                recent_obs = replay_buffer.encode_recent_observation()[np.newaxis, ...]
                q_val = session.run(q_out, feed_dict={obs_t_ph: recent_obs})
                q_action = np.argmax(np.squeeze(q_val)) # greedy
                action = replay_buffer.encode_action(q_action)
            
            
    #        print("action ", action, " exp ", t, " step cnt ", step_count)
            nxt_obs, reward, done, info = replay_buffer._step(env, action)
...
#                print("reward:", reward)
            ...
            if done: # Penalty
                    #time = info['time']
                reward += -1.0
                print("distance ", max_distance, " current distance ",info['distance'], "esion ", e, "episode ", t, " reward ", reward )
...
##                reward = -50
            ...
            if (num_param_updates > learning_starts and
                    num_param_updates % learning_freq == 0 and
                    replay_buffer.can_sample(batch_size)):
                
               # print("step_count ", num_param_updates)
                ...
                if num_param_updates % target_update_freq == 0:
                    session.run(update_target_fn)
   #                 print("update_target_fn ", num_param_updates)
